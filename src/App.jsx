...
// ================= CONFIG =================
export const PLAYER_SPEED = 0.12;       // calm walk
export const RUN_MULTIPLIER = 2.0;      // hold-to-run
...
// ================= CHUNK GENERATOR (ROADS + VEHICLES) =================
export function generateChunk(cx, cz) {
  ...
  // One main road per chunk (horizontal or vertical)
  const horizontal = rand() > 0.5;

  if (biome !== "park") {
    const offset = (rand() - 0.5) * CHUNK_SIZE * 0.6;

    if (horizontal) {
      roads.push({
        from: [cx * CHUNK_SIZE - CHUNK_SIZE / 2, cz * CHUNK_SIZE + offset],
        to:   [cx * CHUNK_SIZE + CHUNK_SIZE / 2, cz * CHUNK_SIZE + offset],
        dir: 0
      });
    } else {
      roads.push({
        from: [cx * CHUNK_SIZE + offset, cz * CHUNK_SIZE - CHUNK_SIZE / 2],
        to:   [cx * CHUNK_SIZE + offset, cz * CHUNK_SIZE + CHUNK_SIZE / 2],
        dir: Math.PI / 2
      });
    }
    ...
    for (let i = 0; i < vCount; i++) {
      ...
      const x = horizontal
        ? THREE.MathUtils.lerp(road.from[0], road.to[0], t)
        : road.from[0];

      const z = horizontal
        ? road.from[1]
        : THREE.MathUtils.lerp(road.from[1], road.to[1], t);
  ...
  return { cx, cz, biome, buildings, npcs, roads, vehicles };
        }
// ================= PLAYER =================
...
export const Road = ({ from, to }) => {
  ...
  return (
    <mesh position={[midX, -0.02, midZ]} rotation={[0, -rot, 0]} receiveShadow>
      <boxGeometry args={[len, 0.05, 2]} />
      <meshStandardMaterial color="#0b1220" />
    </mesh>
  );
};

// Vehicle visuals (hybrid surreal)
export const Vehicle = ({ data }) => {
  ...
  useFrame(() => {
    ...
    // Move along its road using t
    data.t += data.speed;
    if (data.t > 1) data.t = 0;

    const r = data.road;
    const x = THREE.MathUtils.lerp(r.from[0], r.to[0], data.t);
    const z = THREE.MathUtils.lerp(r.from[1], r.to[1], data.t);

    ref.current.position.x = x;
    ref.current.position.z = z;

    // Subtle hover / surreal motion
    if (data.type === "hover") {
      ref.current.position.y = 0.4 + Math.sin(data.t * Math.PI * 2) * 0.1;
    } else if (data.type === "surreal") {
      ref.current.position.y = 0.2 + Math.cos(data.t * Math.PI * 4) * 0.2;
  ...
  const color =
    data.type === "hover" ? "#22d3ee" :
    data.type === "surreal" ? "#a855f7" :
    "#f59e0b";
